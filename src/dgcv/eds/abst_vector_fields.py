...
class abstract_VF_atom:
    def __init__(
        ...
        label: str,
        dual_label=None,
        coframe_identifier={"coframe": None, "index": 0},
    ):
        self.label = label
        ...
        if isinstance(CF, abst_coframe):
            ...
            if dual_label is None or not isinstance(dual_label, str):
                dual_label = "d_" + label
            self.dual_coframe = abst_coframe(
                "_CF_auto_" + label, [dual_label], "_CF_" + label
            )
...
class abstract_VF:
    ...
    def _normalize(self, node):
        ...
        if tag == "bracket":
            ...
            if self.favor_factoring is not True:
                ...
                if X_terms is not None and Y_terms is not None:
                    core = make_sum(
                        [("bracket", xi, yj) for xi in X_terms for yj in Y_terms]
                    )
        ...
        if tag == "ad":
            ...
            if len(node) != 4:
                raise ValueError(
                    "('ad', X, Y, n) requires exactly 3 operands and an exponent n"
                )
    ...
    def _eval_simplify(self, aggressive=False, **kws):
        ...
        Steps:
          ...
          2) Simplify scalar coefficients in every ('scale', c, ·) using simplify(c).
    ...
    def __call__(self, arg):
        ...
        def apply_node(vf_node, zero_form):
            ...
            if t == "bracket":
                # [A,B](g) = A(B(g)) - B(A(g))
                return apply_node(
                    vf_node[1], apply_node(vf_node[2], zero_form)
...
def _vf_apply_node_to_zero_form(vf_node, zero_form):
    ...
    if tag == "bracket":
        # [A,B](g) = A(B(g)) - B(A(g))
        return _vf_apply_node_to_zero_form(
            vf_node[1], _vf_apply_node_to_zero_form(vf_node[2], zero_form)
        ) - _vf_apply_node_to_zero_form(
            vf_node[2], _vf_apply_node_to_zero_form(vf_node[1], zero_form)
        )
        ...
        if tag == "bracket":
            ...
            if my_depth == target_depth:
                ...
                if (
                    ...
                    and L2
                    and L2[0] == "scale"
                    and isinstance(L2[1], (abstract_ZF, zeroFormAtom))
                    and isinstance(R2, tuple)
                    and R2
                    and R2[0] == "scale"
                    and isinstance(R2[1], (abstract_ZF, zeroFormAtom))
                ):
                    f, X = L2[1], L2[2]
                    g, Y = R2[1], R2[2]
...
def VF_factor(vf: abstract_VF, aggressive: bool = False) -> abstract_VF:
    ...
    if all_scale and flat_terms:
        ...
        if all_equal:
            ...
            if all(isinstance(c, numbers.Integral) for c in coeffs):
        ...
        if t[0] == "scale" and is_numeric(t[1]):
            ...
            if child[0] == "bracket":
                ...
                if (
                    isinstance(L, tuple)
                    and isinstance(R, tuple)
                    and L == root_vf._VFify(L)._node
                    and R == root_vf._VFify(R)._node
                ):
...
def abst_lie_bracket(
    X: "abstract_VF | abstract_VF_atom", Y: "abstract_VF | abstract_VF_atom"
) -> abstract_VF:
