...
from ..combinatorics.combinatorics import permSign, shufflings
...
class vector_space_class(dgcv_class):
    def __init__(
        ...
        if not isinstance(dimension, numbers.Integral) or dimension < 0:
            raise TypeError(
                "vector_space_class expected dimension to be a positive int."
            )
        ...
        self.basis = tuple(
            vector_space_element(
                self, [1 if i == j else 0 for j in range(self.dimension)], 1
            )
    ...
    def _repr_latex_(self, raw: bool = False, abbrev: bool = False, **kwargs):
        ...
        return space_display(
            ...
            label=self.label,
            label_tex=None,
            mathfrak_label=False,
            unlabeled_tex=r"\text{Unnamed VS}",
            max_dim=20,
            raw=raw,
            abbrev=abbrev,
    ...
    def check_element_weight(self, element, test_weights=None, flatten_weights=False):
        ...
        """
        if (
            not get_dgcv_category(element) == "vector_space_element"
            or element.vectorSpace != self
        ):
            raise TypeError(
                "Input in `vector_space_class.check_element_weight` must be a `vector_space_element` instance belonging to the `vector_space_class` instance whose `check_element_weight` is being called."
            ) from None
        if not test_weights:
            if not hasattr(self, "grading") or self._gradingNumber == 0:
                raise ValueError(
                    "This vector_space_class instance has no assigned grading vectors."
                ) from None
...
class vector_space_element(dgcv_class):
    def __init__(self, VS, coeffs, valence):
        ...
        if isinstance(coeffs, dict):
            ...
            coeffs = {k: v for k, v in enumerate(coeffs) if v != 0}
        elif get_dgcv_category(coeffs) == "array":
            coeffs = coeffs._data
        else:
            raise (
    ...
    def __eq__(self, other):
        if not isinstance(other, vector_space_element):
            return NotImplemented
        return (
            self.vectorSpace == other.vectorSpace
            and self._coeffs_hash == other._coeffs_hash
            and self.valence == other.valence
        )

    def __hash__(self):
        return hash((self.vectorSpace, self._coeffs_hash, self.valence))
    ...
    def _sympystr(self):
        ...
        if self.vectorSpace.label:
            return (
                f"vector_space_element({self.vectorSpace.label}, coeffs=[{coeffs_str}])"
            )
        else:
    ...
    def contains(self, items, return_basis_coeffs=False, strict_types=True):
        ...
        if (
            get_dgcv_category(items) == "vector_space_element"
    ...
    def dual(self):
        return vector_space_element(
            self.vectorSpace, self.coeffs, (self.valence + 1) % 2
        )
    ...
    def _convert_to_tp(self):
        return tensorProduct(
            (self.dgcv_vs_id,),
            {
                ...
                for j in range(self.vectorSpace.dimension)
            },
            shape="all",
        )
    ...
    def __call__(self, other):
        if (
            not isinstance(other, vector_space_element)
            or other.vectorSpace != self.vectorSpace
            or other.valence == self.valence
        ):
            raise TypeError(
                "`vector_space_element.call()` can only be applied to `vector_space_element` instances with the same vector_space_class but different valence attributes."
            )
        return sum(
            [
                self.coeffs[j] * other.coeffs[j]
                for j in range(self.vectorSpace.dimension)
            ]
        )
    ...
    def __matmul__(self, other):
        ...
        if get_dgcv_category(other) not in {
            ...
            raise TypeError(
                "`@` only supports tensor products between vector_space_elements instances with the same vector_space_class attribute"
            )
        return tensorProduct(
            "_",
            {
                ...
                for j in range(self.vectorSpace.dimension)
                for k in range(other.vectorSpace.dimension)
            },
        )
    ...
    def check_element_weight(self, test_weights=None, flatten_weights=False):
        ...
        return self.vectorSpace.check_element_weight(
...
class tensorProduct:
    def __init__(
        ...
        try:
            result = tensorProduct._process_coeffs_dict(
                coeff_dict,
                shape=shape,
                _amb_prom=_amb_prom,
    ...
    def _process_coeffs_dict(
        ...
        for key, value in coeff_dict.items():
            ...
            if min_degree < 0 or 3 * min_degree > kl:
                min_degree = deg
            if prolongation_type != -1 and all(
                (key[deg] + j) % 2 == 1 for j in key[deg + 1 : 2 * deg]
            ):
        ...
        if replacements:
            ...
            for k, v in processed_dict.items():
                ...
                for nk, nv in replace(k, targets, v).items():
                    final_dict[nk] += nv
            processed_dict = dict(final_dict)
            vector_spaces = [
                vsidx for vsidx in vector_spaces if vsidx not in replacements
            ]
        if shape == "skew" or shape == "symmetric" or callable(shape):
            processed_dict = _shape_filter(
                processed_dict, shape=shape, accumulate=_process_shape_with_accumulation
            )
            if not processed_dict:
                processed_dict[tuple()] = 0
            prolongation_type = -1
    ...
    def homogeneous_components(self):
        return [
            tensorProduct(self.vector_spaces, cd)
            for cd in self.homogeneous_dicts.values()
        ]
    ...
    def free_vectors(self):
        ...
        for idx_t in self.coeff_dict:
            vec_idx = vec_idx | set(
                tuple(zip(idx_t[: len(idx_t) // 3], idx_t[len(2 * idx_t) // 3 :]))
            )
        return set(
            [from_vsr(self.vector_spaces[idx[1]]).basis[idx[0]] for idx in vec_idx]
        )
    ...
    def _compute_weights(
        ...
        if test_weights is None:
            ...
            if isinstance(test_weights, (list, tuple)):
                mw = len(test_weights)
                test_weights = {
                    idx: test_weights for idx in self.vector_spaces
    ...
    def simplify(self):
        ...
        return tensorProduct(self.vector_spaces, new_dict, shape=self.shape)
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, tensorProduct):
            raise TypeError(
                "`+` can only combine `tensorProduct` elements with elements from variants of dgcv vector space type classes."
            )
    ...
    def dual(self):
        ...
        return tensorProduct(
            self.vector_spaces, {keyflip(k): v for k, v in self.coeff_dict.items()}
        )
    ...
    def contract_call(self, other):
        ...
        if isinstance(other, tensorProduct):
            if len(self.vector_spaces) != 1 or tuple(self.vector_spaces) != tuple(
                other.vector_spaces
            ):
                raise ValueError(
                ...
                for key2, value2 in other.coeff_dict.items():
                    ...
                    if key1[(kl1 // 3) - 1] == key2[0]:
                        new_key = (
                            key1[: (kl1 // 3) - 1]
                            + key2[1 : (kl2 // 3)]
                            + key1[(kl1 // 3) : 2 * (kl1 // 3) - 1]
                            + key2[1 + (kl2 // 3) : 2 * kl2 // 3]
                            + key1[2 * (kl1 // 3) : -1]
                            + key2[1 + (2 * kl2 // 3) :]
                        )
    ...
    def _recursion_contract_hom(self, other):
        ...
        if self.prolongation_type == 0:
            ...
            raise TypeError(
                f"`_recursion_contract_hom` does not operate on arguments with mixed `prolongation_type` e.g., {self} has type {self.prolongation_type}"
            )
    ...
    def _recursion_contract(self, other):
        ...
        if isinstance(other, tensorProduct):
            if self.prolongation_type != other.prolongation_type:
                raise type(
                    f"`tensorProduct` contraction is only supported between instances with matching `prolongation types`, not types: {self.prolongation_type} and {other.prolongation_type}"
                )
    ...
    def _bracket(self, other):
        ...
        if isinstance(other, tensorProduct):
            ...
            if (
                self.prolongation_type != other.prolongation_type
                or self.prolongation_type == -1
            ):
                raise ValueError(
                    "`tensorProduct._bracket` requires bracket components to have matching prolongation types."
                )
            ...
            for key1, value1 in self.coeff_dict.items():
                ...
                for key2, value2 in other.coeff_dict.items():
                    ...
                    for idx in range(1, degree1 - 1):  # double check degree-1
                        if (
                            key1[idx] == key2[0] and len(key2) > 3
                            ...
                            valence = (self.prolongation_type,) + (complimentType,) * (
                                degree1 + degree2 - 3
                            )
                    ...
                    for idx in range(1, degree2 - 1):
                        if (
                            key2[idx] == key1[0] and len(key1) > 3
                        ):  # Check index matching before contraction
                            new_value = -value1 * value2
                            k2_start = key2[:idx]
                            k1_start = key1[1:2]
            ...
            for key, value in self.coeff_dict.items():
                kl = len(key)
                if (
                    key[(kl // 3) - 1] == other_index
                ):  # Matching indices for contraction
    ...
    def _bracket_gen(self, other):
        ...
        if isinstance(other, tensorProduct):
            ...
            for key1, value1 in self.coeff_dict.items():
                ...
                degree1 = len(key1) // 3
                bound_k1 = tuple(
                    (key1[idx], key1[idx + degree1], key1[idx + 2 * degree1])
                    for idx in range(degree1)
                )
                for key2, value2 in other.coeff_dict.items():
                    if len(key2) == 0:
                        continue
                    degree2 = len(key2) // 3
                    bound_k2 = tuple(
                        (key2[idx], key2[idx + degree2], key2[idx + 2 * degree2])
                        for idx in range(degree2)
                    )
                    for idx in range(1, degree1 - 1):
                        if (
                            key1[idx] == key2[0]
                            and key1[idx + degree1] != key2[degree2]
                            and key1[idx + 2 * degree1] != key2[2 * degree2]
                            and degree2 > 1
                        ):
                            new_value = value1 * value2
                            ...
                            new_tails = shufflings(k1_tail_inputs, k2_inputs)
                            new_keys = [
                                sum(zip(*(k1_start + k2_start + tuple(tail))), ())
                                for tail in new_tails
                            ]
                            for key in new_keys:
                    ...
                    if degree1 == 1 and degree2 == 1 and key1[1:] == key2[1:]:
                        ...
                        if (
                            key1[degree1 - 1] == key2[0]
                            and key1[2 * degree1 - 1] != key2[degree2]
                            and key1[-1] == key2[2 * degree2]
                        ):
                            new_value = value1 * value2
                        ...
                        if (
                            key2[degree2 - 1] == key1[0]
                            and key2[2 * degree2 - 1] != key1[degree1]
                            and key2[-1] == key1[2 * degree1]
                        ):
                    ...
                    for idx in range(1, degree2 - 1):
                        if (
                            key2[idx] == key1[0]
                            and key2[idx + degree2] != key1[degree1]
                            and key2[idx + 2 * degree2] != key1[2 * degree1]
                            and degree1 > 1
                        ):
                            new_value = -value2 * value1
                            ...
                            new_tails = shufflings(k2_tail_inputs, k1_inputs)
                            new_keys = [
                                sum(zip(*(k2_start + k1_start + tuple(tail))), ())
                                for tail in new_tails
                            ]
                            for key in new_keys:
                                new_dict[key] = new_dict.get(key, 0) + new_value
            ...
            for other_index, other_value in other_as_tensor.coeff_dict.items():
                for key, value in self.coeff_dict.items():
                    deg = len(key) // 3
                    if (
                        key[deg - 1] == other_index[0]
                        and key[2 * deg - 1] == other_index[1]
                        and key[-1] == other_index[2]
                    ):
                        new_value = value * other_value
                        key_truncated = tuple(
                            key[: deg - 1] + key[deg : 2 * deg - 1] + key[2 * deg : -1]
                        )
                    ...
                    if (
                        key[0] == other_index[0]
                        and key[deg] == other_index[1]
                        and key[2 * deg] == other_index[2]
                    ):
                        new_value = -value * other_value
                        key_truncated = tuple(
                            key[1:deg] + key[deg + 1 : 2 * deg] + key[2 * deg + 1 :]
                        )
                        new_dict[key_truncated] = (
                            new_dict.get(key_truncated, 0) + new_value
                        )
        ...
        if deg == 1:
            if get_dgcv_category(other) == "subalgebra_element":
                if k[1] == other.valence:
                    elem = (
                        v * (from_vsr(k[2]).basis[k[0]])
                        if k[1] == 1
                        else v * (from_vsr(k[2]).basis[k[0]].dual())
                    )
                    if k[2] == other.dgcv_vs_id:
                        ...
                        getattr(elem, "ambient", elem).dgcv_vs_id
                        == other.ambient_rep.dgcv_vs_id
                    ):
                    ...
                    if deg2 == 0:
                        ...
                        if k[2] == k2[2]:
                            if k[1] != k2[1]:
                                ...
                                elem2 = (
                                    v2 * (from_vsr(k2[2]).basis[k2[0]])
                                    if k2[1] == 1
                                    else v * (from_vsr(k2[2]).basis[k2[0]].dual())
                                )
                                nd = elem1 * elem2
                            ...
                            if k[2] == _vsr_inh_idx(k[2]):
                                nd = self._contraction_product(
                                    tensorProduct("_", {k2: v2}, _amb_prom=True)
                                )
                            else:
                            ...
                            if k[2] == k2[2 * deg2]:
                                if k[0] == k2[0]:
                                    newDict[
                                        tuple(
                                            k2[j]
                                            for j in range(1, 3 * deg2)
                                            if j not in {deg2, 2 * deg2}
                                        )
                                    ] += v * v2
                            ...
                            if k[2] == k2[-1]:
                                if k[0] == k2[deg2 - 1]:
                                    newDict[
                                        tuple(
                                            k2[j]
                                            for j in range(3 * deg2 - 1)
                                            if j not in {deg2 - 1, 2 * deg2 - 1}
                                        )
                                    ] += -v * v2
            ...
            if get_dgcv_category(other) in {
                ...
                if (
                    _vsr_inh_idx(k[-1]) != k[-1]
                    and k[-1] != other.dgcv_vs_id
                    and k[2 * deg - 1] != other.valence
                    and getattr(other, "ambient_rep", other).dgcv_vs_id
                    == _vsr_inh_idx(k[-1])
                ):
                    return self.ambient_rep._contraction_product(other)
                if (
                    _vsr_inh_idx(k[2 * deg]) != k[2 * deg]
                    and k[deg] != other.valence
                    and k[2 * deg] != other.dgcv_vs_id
                    and getattr(other, "ambient_rep", other).dgcv_vs_id
                    == _vsr_inh_idx(k[2 * deg])
                ):
                    return self.ambient_rep._contraction_product(other)
                newDict = Counter()
                ...
                if k[2 * deg - 1] != other.valence:
                    locElem = (
                        other.ambient_rep
                        if k[-1] == other.algebra.ambient.dgcv_vs_id
                        else other
                    )
                    if k[-1] == locElem.dgcv_vs_id:
                        newDict[
                            tuple(
                                ...
                                if j not in {deg - 1, 2 * deg - 1}
                            )
                        ] += locElem.coeffs[k[deg - 1]] * v
                if k[deg] != other.valence:
                    locElem = (
                        other.ambient_rep
                        if k[2 * deg] == other.algebra.ambient.dgcv_vs_id
                        else other
                    )
                    if k[2 * deg] == locElem.dgcv_vs_id:
                        newDict[
                            tuple(
                                k[j]
                                for j in range(1, 3 * deg)
                                if j not in {deg, 2 * deg}
                            )
                        ] += -locElem.coeffs[k[0]] * v
                return tensorProduct("_", newDict)
            elif get_dgcv_category(other) == "tensorProduct":
                newDict = Counter()
                for key, val in self._unpromoted_spring.items():
                    if val in other.vector_spaces:
                        return self.ambient_rep._contraction_product(other)
                    elif (
                        val in other._vs_spring and key not in other._unpromoted_spring
                    ):
                        return self.ambient_rep._contraction_product(other.ambient_rep)

                bound_k = tuple(
                    (k[idx], k[idx + deg], k[idx + 2 * deg]) for idx in range(deg)
                )
                for k2, v2 in other.coeff_dict.items():
                    deg2 = len(k2) // 3
                    if deg2 == 0:
                        ...
                        if k2[1] == 0:
                            nd = self._contraction_product(
                                v2 * from_vsr(k2[2]).basis[k2[0]].dual()
                            )
                        else:
                            nd = self._contraction_product(
                                v2 * from_vsr(k2[2]).basis[k2[0]]
                        ...
                        bound_k2 = tuple(
                            (k2[idx], k2[idx + deg2], k2[idx + 2 * deg2])
                            for idx in range(deg2)
                        )
                        for idx in range(1, deg - 1):
                            if (
                                k[idx] == k2[0]
                                and k[idx + deg] != k2[deg2]
                                and k[idx + 2 * deg] == k2[2 * deg2]
                            ):
                                new_value = v * v2
                        ...
                        for idx in range(1, deg2 - 1):
                            if (
                                k2[idx] == k[0]
                                and k2[idx + deg2] != k[deg]
                                and k2[idx + 2 * deg2] == k[2 * deg]
                            ):
                                new_value = -v2 * v
                                k2_start = bound_k2[:idx]
        ...
        for t1 in terms1:
            for t2 in terms2:
                ...
                if deg1 != deg2:
                    continue
                if (
                    all(j == k for j, k in zip(cd1[:deg1], cd2[:deg2]))
                    and all(
                        j + k == 1
                        for j, k in zip(cd1[deg1 : 2 * deg1], cd2[deg2 : 2 * deg2])
                    )
                    and all(j == k for j, k in zip(cd1[2 * deg1 :], cd2[2 * deg2 :]))
                ):
    ...
    def __call__(
        ...
        for arg in args:
            ...
            for t in tidx:
                if t not in self.vector_spaces and _vsr_inh_idx(t) in parents:
                    return tensorProduct("_", self.coeff_dict, _amb_prom=True).__call__(
                        *args,
                        contract_from_left=contract_from_left,
                        demote_to_VS_when_possible=demote_to_VS_when_possible,
                    )
        ...
        if get_dgcv_category(elem) == "tensorProduct":
            if elem.max_degree == 1 and elem.min_degree == 1:
                if any(
                    (vs != _vsr_inh_idx(vs) and _vsr_inh_idx(vs) in self.vector_spaces)
                    for vs in elem.vector_spaces
                ):
            ...
            if kvs[idx] == _vsr_inh_idx(
                elem.dgcv_vs_id
            ) and elem.dgcv_vs_id != _vsr_inh_idx(elem.dgcv_vs_id):
...
def _shape_filter(
    ...
    def skew_filter(key: tuple):
        deg = len(key)
        split = tuple(
            zip(key[: deg // 3], key[deg // 3 : 2 * deg // 3], key[2 * deg // 3 :])
        )
        if len(split) != len(set(split)):
            return
        sign, new_split = permSign(split, returnSorted=True)
        return tuple(x for t in zip(*new_split) for x in t), sign
    ...
    def symmetric_filter(key: tuple):
        ...
        new_split = sorted(split)
        return tuple(x for t in zip(*new_split) for x in t), 1
...
# creator functions
# -----------------------------------------------------------------------------
def createVectorSpace(obj, label, basis_labels=None, grading=None, verbose=False):
    """
    ...
    if isinstance(obj, vector_space_class):
        ...
        isinstance(el, vector_space_element) for el in obj
    ):
        if verbose:
