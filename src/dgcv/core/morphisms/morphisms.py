...
class homomorphism(dgcv_class):
    def __init__(self, domain, codomain, basis_image=[]):
        ...
        if isinstance(codomain, (list, tuple)):
            if all(get_dgcv_category(elem) in classes for elem in codomain):
                ...
                raise TypeError(
                    "If providing the `codomain` parameter as a list reprenting a tensor product of vector spaces then its elements must be vector space class types (or similar, e.g., `algebra_class` etc.)"
                )
        ...
        if (
            not isinstance(basis_image, (list, tuple))
            or len(basis_image) != domain.dimension
        ):
            raise TypeError(
                f"`basis_image` parameter should be a list of elements in the codomain. Its length must match the dimension of the domain. Recieved {basis_image} which should have length {domain.dimension}"
            )
        newBI = []
        ...
        def _decomp(tp):
            ...
            tps = [
                var * elem for var, elem in zip(vars, codomain.tensor_representation)
            ]
            if len(tps) == 0:
                return tuple()
        ...
        all_zero = True
        codomcat = get_dgcv_category(codomain) == "tensor_proxy"
        ...
        if not self._zero_map:
            self.tensor_representation = (
                0 * codomain.basis[0] @ domain.basis[0].dual()
            ) + sum(e1 @ e2.dual() for e1, e2 in zip(newBI, domain.basis))
        else:
            self.tensor_representation = 0 * codomain.basis[0] @ domain.basis[0].dual()
        self._alg_hom = None
    ...
    def subs(self, *args, **kwargs):
        def f(x):
            if x is None:
                return None
            return subs(x, *args, **kwargs)

        return self.apply(f, in_place=False, skip_none=True)

    @property
    def _alg_hom_eqns(self):
        skew = (
            getattr(self.domain, "is_skew_symmetric", lambda: False)()
            and getattr(self.codomain, "is_skew_symmetric", lambda: False)()
        )
        eqns, basis = [], self.domain.basis
        for idx, x1 in enumerate(basis):
            start = idx + 1 if skew else 0
            for x2 in basis[start:]:
                eqns.append(self(x1 * x2) - self(x1) * self(x2))
        return eqns

    @property
    def is_algebra_homomorphism(self):
    ...
    def _alg_der_eqns(self):
        ...
        for idx, x1 in enumerate(basis):
            start = idx + 1 if skew else 0
            for x2 in basis[start:]:
                newe = self(x1 * x2) - self(x1) * x2 - x1 * self(x2)
                eqns += list(newe.coeff_dict.values())
        return eqns

    @property
    def is_algebra_derivation(self):
    ...
    def _repr_latex_(self, raw=False, **kwargs):
        if getattr(self, "_zero_map", False):
            return r"$0$"
        return self.tensor_representation._repr_latex_(raw=raw, **kwargs)
...
class _fast_tensor_proxy:
    ...
    def contains(self, elem, *args):
        ...
        if isinstance(elem, (list, tuple)):
            if len(elem) == self.degree and all(
                vs.contains(el, strict_types=False)
                for vs, el in zip(self.vector_spaces, elem)
            ):
                return True
            else:
                return False
        if (
            get_dgcv_category(elem) == "tensorProduct"
            and elem.max_degree == self.degree
